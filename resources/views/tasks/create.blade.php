...
@section('content')
    <form action="{{ route('tasks.store') }}" method="POST" class="flex flex-col gap-3">
        ...
        <fieldset>
            ...
            <input value="{{ old('title') }}" type="text" name="title" id="title" class="border-1 border-slate-200 py-2 px-4 bg-white outline-none w-full transition-all ease-in-out duration-300 focus:border-slate-500"/>
            @error('title')
        ...
        <fieldset>
            ...
            <textarea name="description" id="description" rows="3" class="border-1 border-slate-200 py-2 px-4 bg-white outline-none w-full transition-all ease-in-out duration-300 focus:border-slate-500">{{ old('description') }}</textarea>
            @error('description')
                <span class="text-red-500">{{$message}}</span>
            @enderror
        </fieldset>
        <fieldset>
            ...
            <textarea name="long_description" id="long_description" rows="10" class="border-1 border-slate-200 py-2 px-4 bg-white outline-none w-full transition-all ease-in-out duration-300 focus:border-slate-500">{{ old('long_description') }}</textarea>
